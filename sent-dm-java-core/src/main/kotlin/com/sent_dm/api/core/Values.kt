...
sealed class JsonField<out T : Any> {
    ...
    fun asStringOrThrow(): String =
        asString().orElseThrow { SentDmInvalidDataException("Value is not a string") }
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw SentDmInvalidDataException("`$name` is not set")
            is JsonNull -> throw SentDmInvalidDataException("`$name` is null")
            else -> throw SentDmInvalidDataException("`$name` is invalid, received $this")
        }

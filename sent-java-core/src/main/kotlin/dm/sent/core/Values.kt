...
sealed class JsonField<out T : Any> {
    ...
    fun asStringOrThrow(): String =
        asString().orElseThrow { SentInvalidDataException("Value is not a string") }
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw SentInvalidDataException("`$name` is not set")
            is JsonNull -> throw SentInvalidDataException("`$name` is null")
            else -> throw SentInvalidDataException("`$name` is invalid, received $this")
        }

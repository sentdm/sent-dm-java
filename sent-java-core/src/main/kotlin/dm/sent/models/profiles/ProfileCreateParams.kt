 ...
 *
 * Every profile owns its own WhatsApp Business Account — accounts are never shared between profiles
 * or inherited from the organization. Provide a `whatsapp_business_account` object with `waba_id`,
 * `phone_number_id`, and `access_token`. Obtain these from Meta Business Manager by creating a
 * System User with `whatsapp_business_messaging` and `whatsapp_business_management` permissions.
 *
 * Omit the field and the profile is created without WhatsApp, staying incomplete until it has an
 * account of its own.
 *
 * ## Brand
 *
...
private constructor(
     ...
     * SDK that assigns them keeps compiling, which is the compatibility this exists for.
     * Deliberately not refused either — a 400 would break an integration that is otherwise working,
     * and the capability they ask for is gone either way. Same rule as SendingPhoneNumberProfileId.
     *
     * The read is what makes this survivable: every profile reports all four as false, so a caller
     * that checks its own write can see it did not take. Requests carrying one are logged, so we
     * can tell when nobody sends them any more and the fields can go for real.
     *
    ...
    class Builder internal constructor() {
         ...
         *
         * Bound rather than dropped so the properties survive on the wire and in a generated
         * client: an SDK that assigns them keeps compiling, which is the compatibility this exists
         * for. Deliberately not refused either — a 400 would break an integration that is otherwise
         * working, and the capability they ask for is gone either way. Same rule as
         * SendingPhoneNumberProfileId.
         *
         * The read is what makes this survivable: every profile reports all four as false, so a
         * caller that checks its own write can see it did not take. Requests carrying one are
         * logged, so we can tell when nobody sends them any more and the fields can go for real.
         */
        @Deprecated("deprecated")
    ...
    private constructor(
        ...
        class Builder internal constructor() {
             ...
             *
             * Bound rather than dropped so the properties survive on the wire and in a generated
             * client: an SDK that assigns them keeps compiling, which is the compatibility this
             * exists for. Deliberately not refused either — a 400 would break an integration that
             * is otherwise working, and the capability they ask for is gone either way. Same rule
             * as SendingPhoneNumberProfileId.
             *
             * The read is what makes this survivable: every profile reports all four as false, so a
             * caller that checks its own write can see it did not take. Requests carrying one are
             * logged, so we can tell when nobody sends them any more and the fields can go for
             * real.
             */
            @Deprecated("deprecated")
            fun allowContactSharing(allowContactSharing: Boolean?) =
        ...
        override fun toString() =
            "Body{allowContactSharing=$allowContactSharing, allowTemplateSharing=$allowTemplateSharing, billingContact=$billingContact, billingModel=$billingModel, brand=$brand, description=$description, icon=$icon, inheritContacts=$inheritContacts, inheritTcrBrand=$inheritTcrBrand, inheritTcrCampaign=$inheritTcrCampaign, inheritTemplates=$inheritTemplates, name=$name, paymentDetails=$paymentDetails, sandbox=$sandbox, shortName=$shortName, whatsappBusinessAccount=$whatsappBusinessAccount, additionalProperties=$additionalProperties}"
    }

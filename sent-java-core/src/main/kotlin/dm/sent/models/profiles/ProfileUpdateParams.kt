 ...
 * ignored**. Sender borrowing is gone: a profile cannot send from another profile's number, because
 * two profiles behind one sender makes an inbound reply and a delivery receipt ambiguous about
 * whose they are.
 *
 * Sending either **changes nothing and still returns `200`** — they are kept on the contract so an
 * existing integration keeps working. Reads carry both keys too and always answer `null`, which is
 * how you can confirm the value did not take.
 *
...
private constructor(
    ...
    /**
     * Deprecated. Accepted and ignored. Contact and template sharing between sender profiles is
     * gone — a profile sees only what it owns, and the organization still sees all of its profiles'
     * contacts and templates through read-time widening. The four columns behind these flags were
     * dropped by M260720120000.
     *
     * Retired the same way as SendingPhoneNumberProfileId, and for the same reason: the properties
     * stay bound so an SDK that assigns them keeps compiling, and a 400 would break a working
     * integration over a capability that is gone regardless. Every profile reports all four as
     ...
     *   responded with an unexpected value).
     */
    @Deprecated("deprecated") fun inheritContacts(): Optional<Boolean> = body.inheritContacts()
     ...
     *   responded with an unexpected value).
     */
    @Deprecated("deprecated") fun inheritTemplates(): Optional<Boolean> = body.inheritTemplates()
    ...
    /**
     * Deprecated. Accepted and ignored. Sender borrowing is gone: a profile cannot send from
     * another profile's SMS number. Supplying this changes nothing and the request still succeeds.
     *
     * Bound rather than dropped so the property survives on the wire and in a generated client — an
     * SDK that assigns it keeps compiling, which is the compatibility this exists for. It is
     * deliberately not refused: a 400 here would break an integration that is otherwise working,
     * and the capability it asks for is gone either way.
     *
     * The trade-off, stated plainly. A caller asking for borrowing is told it succeeded when
     * nothing happened. What makes that survivable is the read: sending_phone_number_profile_id
     * comes back null on every profile, so a caller that checks its own write can see it did not
     * take. Every request that carries one is logged, so we can tell when nobody is sending it any
     * more and the field can go for real.
     *
     * Give the profile a sender of its own instead: POST /v3/channels/sms with the x-profile-id
     * header naming it.
     *
     * @throws SentInvalidDataException if the JSON field has an unexpected type (e.g. if the server
     *   responded with an unexpected value).
     ...
     */
    @Deprecated("deprecated") fun _inheritContacts(): JsonField<Boolean> = body._inheritContacts()
     ...
     * type.
     */
    @Deprecated("deprecated") fun _inheritTemplates(): JsonField<Boolean> = body._inheritTemplates()
    ...
    class Builder internal constructor() {
        ...
        /**
         * Deprecated. Accepted and ignored. Contact and template sharing between sender profiles is
         * gone — a profile sees only what it owns, and the organization still sees all of its
         * profiles' contacts and templates through read-time widening. The four columns behind
         * these flags were dropped by M260720120000.
         *
         * Retired the same way as SendingPhoneNumberProfileId, and for the same reason: the
         * properties stay bound so an SDK that assigns them keeps compiling, and a 400 would break
         * a working integration over a capability that is gone regardless. Every profile reports
         * all four as false, so a caller that checks its own write can see it did not take.
         */
        @Deprecated("deprecated")
        ...
        /**
         * Deprecated. Accepted and ignored. Sender borrowing is gone: a profile cannot send from
         * another profile's SMS number. Supplying this changes nothing and the request still
         * succeeds.
         *
         * Bound rather than dropped so the property survives on the wire and in a generated client
         * — an SDK that assigns it keeps compiling, which is the compatibility this exists for. It
         * is deliberately not refused: a 400 here would break an integration that is otherwise
         * working, and the capability it asks for is gone either way.
         *
         * The trade-off, stated plainly. A caller asking for borrowing is told it succeeded when
         * nothing happened. What makes that survivable is the read: sending_phone_number_profile_id
         * comes back null on every profile, so a caller that checks its own write can see it did
         * not take. Every request that carries one is logged, so we can tell when nobody is sending
         * it any more and the field can go for real.
         *
         * Give the profile a sender of its own instead: POST /v3/channels/sms with the x-profile-id
         * header naming it.
         */
    ...
    private constructor(
        ...
        class Builder internal constructor() {
             ...
             * profiles is gone — a profile sees only what it owns, and the organization still sees
             * all of its profiles' contacts and templates through read-time widening. The four
             * columns behind these flags were dropped by M260720120000.
             *
             * Retired the same way as SendingPhoneNumberProfileId, and for the same reason: the
             * properties stay bound so an SDK that assigns them keeps compiling, and a 400 would
             * break a working integration over a capability that is gone regardless. Every profile
             * reports all four as false, so a caller that checks its own write can see it did not
             * take.
             */
            @Deprecated("deprecated")
            fun allowContactSharing(allowContactSharing: Boolean?) =
            ...
            /**
             * Deprecated. Accepted and ignored. Sender borrowing is gone: a profile cannot send
             * from another profile's SMS number. Supplying this changes nothing and the request
             * still succeeds.
             *
             * Bound rather than dropped so the property survives on the wire and in a generated
             * client — an SDK that assigns it keeps compiling, which is the compatibility this
             * exists for. It is deliberately not refused: a 400 here would break an integration
             * that is otherwise working, and the capability it asks for is gone either way.
             *
             * The trade-off, stated plainly. A caller asking for borrowing is told it succeeded
             * when nothing happened. What makes that survivable is the read:
             * sending_phone_number_profile_id comes back null on every profile, so a caller that
             * checks its own write can see it did not take. Every request that carries one is
             * logged, so we can tell when nobody is sending it any more and the field can go for
             * real.
             *
    ...
    private constructor(
        ...
        private constructor(
            ...
            override fun toString() =
                "Compliance{brandRelationship=$brandRelationship, vertical=$vertical, destinationCountries=$destinationCountries, isTcrApplication=$isTcrApplication, notes=$notes, phoneNumberPrefix=$phoneNumberPrefix, additionalProperties=$additionalProperties}"
        }

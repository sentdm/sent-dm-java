...
private constructor(
     ...
     *
     * Still required of a caller, and consulted by nothing. It named the signup path that produced
     * the campaign, was written to a column no query filters on, no TCR payload carries and no fee
     * or status decision reads, and is now defaulted at the entity instead. It stays required so
     * that a request which was valid before is still valid — dropping it would be the
     * client-visible change, not keeping it.
     *
     * @throws SentInvalidDataException if the JSON field has an unexpected type or is unexpectedly
     *   missing or null (e.g. if the server responded with an unexpected value).
     */
